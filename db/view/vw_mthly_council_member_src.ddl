...
CREATE OR ALTER VIEW vw_mthly_council_member_src
AS
...
(
    ...
    SELECT cm.COUNCIL_MEMBER_ID
     ...
     WHERE (
                YEAR(cm.CREATE_DATE) * 100 + MONTH(cm.CREATE_DATE) = (
                                     SELECT YEAR(last_run_date) * 100 + MONTH(last_run_date)
                                       FROM jobcontrol 
                                      WHERE name = 'DIM_COUNCIL_MEMBER'
                                        AND active_ind = 1
                                        AND runnable_ind = 1
                                   )
             OR YEAR(cm.LAST_UPDATE_DATE) * 100 + MONTH(cm.LAST_UPDATE_DATE) = (
                                     SELECT YEAR(last_run_date) * 100 + MONTH(last_run_date)
                                       FROM jobcontrol 
                                      WHERE name = 'DIM_COUNCIL_MEMBER'
                                        AND active_ind = 1
...
(
    ...
    SELECT tcs.COUNCIL_MEMBER_ID
     ...
     WHERE (
                YEAR(tcs.CREATE_DATE) * 100 + MONTH(tcs.CREATE_DATE) = (
                                     SELECT YEAR(last_run_date) * 100 + MONTH(last_run_date)
                                       FROM jobcontrol 
                                      WHERE name = 'DIM_COUNCIL_MEMBER'
                                        AND active_ind = 1
...
(
    ...
    SELECT DISTINCT
     ...
     WHERE cmura.REQUESTED_ACTION = 'EB'
       ...
       AND (
                YEAR(cmura.CREATE_DATE) * 100 + MONTH(cmura.CREATE_DATE) = (
                                     SELECT YEAR(last_run_date) * 100 + MONTH(last_run_date)
                                       FROM jobcontrol 
                                      WHERE name = 'DIM_COUNCIL_MEMBER'
                                        AND active_ind = 1
...
(
    ...
    SELECT DISTINCT
     ...
     WHERE cmr.Product_Type_ID = 3  -- Consultation rates as this is the base rate used
       ...
       AND (
                YEAR(cmr.CREATE_DATE) * 100 + MONTH(cmr.CREATE_DATE) = (
                                     SELECT YEAR(last_run_date) * 100 + MONTH(last_run_date)
                                       FROM jobcontrol 
                                      WHERE name = 'DIM_COUNCIL_MEMBER'
                                        AND active_ind = 1
             ...
             OR YEAR(cmr.LAST_UPDATE_DATE) * 100 + MONTH(cmr.LAST_UPDATE_DATE) = (
                                     SELECT YEAR(last_run_date) * 100 + MONTH(last_run_date)
                                       FROM jobcontrol 
                                      WHERE name = 'DIM_COUNCIL_MEMBER'
                                        AND active_ind = 1
...
(
    ...
    SELECT DISTINCT
     ...
     WHERE (
                YEAR(cmjfrh.CREATE_DATE) * 100 + MONTH(cmjfrh.CREATE_DATE) = (
                                     SELECT YEAR(last_run_date) * 100 + MONTH(last_run_date)
                                       FROM jobcontrol 
                                      WHERE name = 'DIM_COUNCIL_MEMBER'
                                        AND active_ind = 1
             ...
             OR YEAR(cmjfrh.LAST_UPDATE_DATE) * 100 + MONTH(cmjfrh.LAST_UPDATE_DATE) = (
                                     SELECT YEAR(last_run_date) * 100 + MONTH(last_run_date)
                                       FROM jobcontrol 
                                      WHERE name = 'DIM_COUNCIL_MEMBER'
                                        AND active_ind = 1
...
(
    ...
    SELECT cmfh.COUNCIL_MEMBER_ID
     ...
     WHERE (
                YEAR(cmfh.CREATE_DATE) * 100 + MONTH(cmfh.CREATE_DATE) = (
                                     SELECT YEAR(last_run_date) * 100 + MONTH(last_run_date)
                                       FROM jobcontrol 
                                      WHERE name = 'DIM_COUNCIL_MEMBER'
                                        AND active_ind = 1
             ...
             OR YEAR(cmfh.FLAG_RELATION_LAST_UPDATE_DATE) * 100 + MONTH(cmfh.FLAG_RELATION_LAST_UPDATE_DATE) = (
                                     SELECT YEAR(last_run_date) * 100 + MONTH(last_run_date)
                                       FROM jobcontrol 
                                      WHERE name = 'DIM_COUNCIL_MEMBER'
                                        AND active_ind = 1
